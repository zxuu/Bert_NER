...
class BertNERModel(nn.Module):
    ...
    def __init__(self, ner_labels, pretrained_name='bert-base-uncased'):
        ...
        self.bert = BertModel.from_pretrained(pretrained_name,
                                              return_dict=True, cache_dir='../models/bert-base-uncased')
...
def load_sentence_nertags(data_path, train_ratio=0.8):
    ...
    :param data_path:
    :param train_ratio:
    :return: [(sent, ner_labels)(sent, ner_labels)(sent, ner_labels)(sent, ner_labels)(sent, ner_labels)]
    '''
    all_data = []
    ...
    for i, labels in enumerate(all_labels):
        ...
        for word_idx in tokenized_inputs.word_ids(batch_index=i):
            ...
            if word_idx is None:
                ...
                # 把标签设置到每个词的第一个token上
                label_ids.append(labels[word_idx])    # 子词标签和母词标签一样
...
data_path = "../data/ner/conll2003.jsonl.txt"  # 数据所在地址
...
model = BertNERModel(9, pretrained_model_name)
# 固定写法，将模型加载到device上，
...
# os.makedirs(checkpoints_dirname, exist_ok=True)
torch.save(model,'en_uncased.pth')